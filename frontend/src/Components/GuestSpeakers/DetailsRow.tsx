...
const UserDetailsRow = ({field, value, variableName}: Props) => {
  ...
  const onClickCheck = async () => {
    ...
    if (field === "About" || field === "Session Description") {
        formattedValue = (valueState as string).split("\n\n").map((item: string) => item.trim());
    }
    ...
    if ((field === "Website" || field === "Session Recording" || field === "Photo") && validURL(valueState) === false) {
        showNotification("Please enter a valid url.", "error");
        return;
    }

    await axios.put(`${host}/api/auth/guest-speaker/${id}`, {
        id,
        field: variableName,
  ...
  const verifyInputField = (field: string) => {
    if(field === "Date") {
      return <Input type="date" value={valueState} flex={1} h="32px" onChange={onChange}/>
    }

    if(field === "Session Description" || field === "About") {
        return <Textarea flex={1} h="32px" value={valueState} onChange={onChange} />
    } 

    if (field === "Website" || field === "Session Recording" || field === "Photo") {
        return <Input type="url" flex={1} h="32px" value={valueState} onChange={onChange}/>
    }

    return <Input type="text" flex={1} h="32px" value={valueState} onChange={onChange}/>
  }
  ...
  const verifyTextField = (field: string) => {
    if (field === "Website" || field === "Session Recording" || field === "Photo") {
      const truncateString = (str: string, num: number) => {
        ...
        return <Text flex={1} h="32px" onChange={onChange}>
    ...
    if (field === "Date" && valueState.length < 11) {
        ...
        return <Text flex={1} h="32px">{dateValue.toLocaleDateString('en-US', options)}</Text>;
      }

    return <Text flex={1} lineHeight='32px'>{(valueState)}</Text>
  }
